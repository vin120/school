...
<div id="templatemo_wrapper">
    ...
    <p>你现在的位置是:<a href="index.php">首页</a> > <a href="CY_news.php">入驻通知</a></p>
    ...
    <?php 
            ...
            if(!empty($id))
            {
                $sql = "SELECT * FROM  `CY_news` WHERE  id = $id";
                $row = fetchOne($sql);
                $title = $row['title'];
                $author = $row['author'];
                $times = $row['times'];
    ...
    <?php
            ...
            {   
                ...
                $sql  = "SELECT * FROM `CY_news` ORDER BY  id DESC LIMIT {$offset},{$pageSize}";
                $rows = fetchAll($sql);
                ...
                foreach ($rows as $row ) 
                {
                    ...
                    $time = explode(" ",$times); 
            ?>
                <h4 align="left" ><a href = "CY_news.php?id=<?php echo $id?>"><?php echo $title ; ?></a><div style="float:right"><?php echo $time[0];?></div></h4>
